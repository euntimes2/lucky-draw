...
export class RouletteRenderer {
  ...
  async init() {
    ...
    this._canvas.className = 'original-roulette-canvas';
    this._container.appendChild(this._canvas);

    const resizing = (entries?: ResizeObserverEntry[]) => {
      const realSize = entries ? entries[0].contentRect : this._container.getBoundingClientRect();
      const fallbackWidth = window.innerWidth || 1280;
      const fallbackHeight = window.innerHeight || 720;
      const realWidth = Math.max(realSize.width || fallbackWidth, 1);
      const realHeight = Math.max(realSize.height || fallbackHeight, 1);
      const width = Math.max(realWidth / 2, 640);
      const height = (width / realWidth) * realHeight;
      this._canvas.width = width;
      this._canvas.height = height;
      this.sizeFactor = width / realWidth;
    };
